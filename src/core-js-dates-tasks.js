...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const countMs = endMs - startMs;

  const countDays = Math.ceil(countMs / (1000 * 60 * 60 * 24)) + 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    ...
    cyclePosition = (cyclePosition + 1) % (countWorkDays + countOffDays);
  }
